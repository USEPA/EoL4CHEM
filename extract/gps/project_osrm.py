...
class OSRM_API:
    ...
    def sea_port_lists(self, Lat, Long):
        ...
        Ports['Distance'] = Ports.apply(lambda x:
                                            self.harvesine_formula(x['Latitude'], x['Longitude'],
                                             Lat, Long),
                                        axis=1)
    ...
    def maritime_transport(self, Lat_1, Long_1, Lat_2, Long_2, N_Times):
        ...
        distance = self.harvesine_formula(Lat_port_1, Lon_port_1, Lat_port_2, Lon_port_2)
        Maritime_distance = distance
        Land_1, _ =  self.request_directions(Lat_1, Long_1, Lat_port_1, Lon_port_1, Times=N_Times)
        distance = distance + Land_1
        Lan_2, _ = self.request_directions(Lat_port_2, Lon_port_2, Lat_2, Long_2, Times=N_Times)
        ...
        if N_Times < 5:
            try:
                ...
                if result.status_code == 200:
                    distance = round(result.json()['routes'][0]['legs'][0]['distance']/1000, 4)
                    Maritime_distance = 0.0
                else:
                    N_Times += 1
                    distance, Maritime_distance = self.maritime_transport(Lat_1, Long_1, Lat_2, Long_2, N_Times)
            except IndexError:
                N_Times += 1
                distance, Maritime_distance = self.maritime_transport(Lat_1, Long_1, Lat_2, Long_2, N_Times)
        else:
    ...
    def harvesine_formula(self, Lat_1, Long_1, Lat_2, Long_2):
        ...
        distance = round(2*Average_earth_radius*math.atan2(math.sqrt(a), math.sqrt(1 - a)), 4)
        return distance
