...
class TRI_EoU:
    ...
    def _file(self, df):
        ...
        NAICS.rename(columns = {'NAICS Title': 'SENDER TRI PRIMARY NAICS TITLE',
                                'NAICS Code': 'SENDER TRI PRIMARY NAICS CODE'},
                                inplace = True)
        columns = ['REPORTING YEAR', 'SENDER TRI ID', 'SENDER NAME', 'SENDER STREET', 'SENDER CITY',
                    'SENDER COUNTY', 'SENDER STATE', 'SENDER ZIP', 'SENDER TRI PRIMARY NAICS CODE',
                    'SENDER TRI PRIMARY NAICS TITLE', 'TRI CHEMICAL ID NUMBER', 'TRI CHEMICAL NAME',
                    'TRI CLASSIFICATION', 'METAL INDICATOR', 'SENDER CONDITION OF USE', 'UNIT',
                    'QUANTITY TRANSFER OFF-SITE', 'RELIABILITY OF OFF-SITE TRANSFER', 'OFF-SITE WASTE MANAGEMENT', 'RECEIVER RCRAInfo ID',
                    'RECEIVER NAME', 'RECEIVER STREET', 'RECEIVER CITY', 'RECEIVER COUNTY', 'RECEIVER STATE', 'RECEIVER ZIP']
        ...
        Cols_Transfer = df.columns[22::2].tolist()
        ...
        # Renaming
        df.rename(columns = {'TRIFID': 'SENDER TRI ID', 'FACILITY NAME': 'SENDER NAME', 'FACILITY STREET': 'SENDER STREET', \
                            'FACILITY CITY': 'SENDER CITY', 'FACILITY COUNTY': 'SENDER COUNTY', 'FACILITY STATE': 'SENDER STATE', \
                            'FACILITY ZIP CODE': 'SENDER ZIP', 'CAS NUMBER': 'TRI CHEMICAL ID NUMBER', 'CHEMICAL NAME':  'TRI CHEMICAL NAME', \
                            'CLASSIFICATION': 'TRI CLASSIFICATION', 'UNIT OF MEASURE': 'UNIT', 'PRIMARY NAICS CODE': 'SENDER TRI PRIMARY NAICS CODE', \
                ...
                , inplace = True)
        # Organizing flows
        Cols_Facility = df.iloc[:, 0:22].columns.tolist()
        TRI = pd.DataFrame()
        ...
        TRI = pd.merge(TRI, NAICS, on = 'SENDER TRI PRIMARY NAICS CODE',
                        how = 'left')
        TRI = TRI[columns]
        return TRI
    ...
    def Generate_DataFrame(self):
        ...
        for key in self.Files:
            ...
            if key == '1b':
                ...
                TRI_needed[key]['SENDER CONDITION OF USE'] = TRI_needed[key][Cols_CoU].apply(lambda row: ' + '.join(val.capitalize() \
        ...
        del TRI_needed
        TRI_EoU = self._file(TRI_merged)
        if not os.path.isdir(self._dir_path + '/' + self.year):
            os.mkdir(self._dir_path + '/' + self.year)
        TRI_EoU.to_csv(self._dir_path + '/' + self.year + '/TRI_' + self.year + '_EoU.csv', sep = ',',
                        index = False)
    ...
    def SRS_Search(self):
        ...
        TRI = pd.read_csv(self._dir_path + '/' + self.year + '/TRI_' + self.year + '_EoU.csv',
                        sep = ',', header = 0,
        ...
        SRS_TRI_RCRA['SRS CHEMICAL ITN'] = SRS_TRI_RCRA['Internal Tracking Number']
        ...
        Columns =  Columns[0:10] + [SRS_TRI_Columns[6], Columns[10]] + SRS_TRI_Columns[7:9] + [Columns[11]] + \
                    SRS_TRI_Columns[0:6] + Columns[12:]
        TRI_RCRAInfo_Merged = TRI_RCRAInfo_Merged[Columns].drop_duplicates(keep = 'first')
        TRI_RCRAInfo_Merged.to_csv(self._dir_path + '/' + self.year + '/TRI_SRS_' + self.year + '_EoU.csv',
                                    sep = ',', index = False)
    ...
    def FRS_Search(self):
        # Calling database
        TRI = pd.read_csv(self._dir_path + '/' + self.year + '/TRI_SRS_' + self.year + '_EoU.csv',
                        sep = ',', header = 0,
                        dtype = {'REPORTING YEAR':'int',
                                'SENDER ZIP':'object',
                                'RECEIVER ZIP':'object',
                                'SENDER TRI PRIMARY NAICS CODE':'object',
        ...
        df3['RECEIVER FRS ID'] = df3['REGISTRY_ID']
        df3['RECEIVER FRS ID'] = df3['RECEIVER FRS ID'].astype(int).apply(lambda x: abs(x))
        ...
        TRI_IDs['RECEIVER FRS ID'] = TRI_IDs['REGISTRY_ID']
        ...
        TRI_RCRAInfo_TRI = pd.merge(E_TRI, TRI_RCRAInfo,
                                left_on = 'REGISTRY_ID',
                                 right_on = 'RECEIVER FRS ID',
                                 how = 'right')
        TRI_RCRAInfo_TRI['RECEIVER TRI ID'] = TRI_RCRAInfo_TRI['PGM_SYS_ID']
        TRI_RCRAInfo_TRI = TRI_RCRAInfo_TRI[pd.notnull(TRI_RCRAInfo_TRI['RECEIVER RCRAInfo ID']) \
                                | pd.notnull(TRI_RCRAInfo_TRI['RECEIVER TRI ID'])]
        TRI_RCRAInfo_TRI.drop(['REGISTRY_ID', 'PGM_SYS_ID'], axis = 1, inplace = True)
        Columns = Columns[0:28] + ['RECEIVER FRS ID', 'RECEIVER TRI ID'] + Columns[28:]
        ...
        TRI_RCRAInfo_TRI.to_csv(self._dir_path + '/' + self.year + '/TRI_SRS_FRS_' + self.year + '_EoU.csv',
                             sep = ',', index = False)
    ...
    def _Off_tracker(self, Management, Receiver_FRS_ID, Chemical_SRS_ID, RCRA_ID, Track, df_WM):
        ...
        grouping = ['FOR WHAT IS TRANSFERRED', 'RECEIVER FRS ID', 'SENDER FRS ID']
        if RCRA_ID:
            Quantity_received = Track.loc[pd.notnull(Track['QUANTITY RECEIVED'])]
            Quantity_received.drop(columns =  ['QUANTITY TRANSFERRED', 'RECEIVER TRI ID', 'SENDER FRS ID', 'RELIABILITY'], inplace = True)
        ...
        if not Source.empty:
            ...
            Source['TEMPORAL CORRELATION OF MAXIMUM POSSIBLE FLOW'] = Source.apply(lambda row: \
                                                    ...
                                                    axis = 1)
            Source.drop(columns = ['QUANTITY TRANSFERRED', 'SENDER FRS ID', 'RELIABILITY', 'Year_difference'], inplace = True)
            ...
            Paths.rename(columns = {'RECEIVER FRS ID': 'FINAL HANDLER FRS ID',
                                        'RECEIVER TRI ID': 'FINAL HANDLER TRI ID'},
                            inplace = True)
            if Management == 'Transfer to waste broker for energy recovery':
            ...
            while (not Source.empty) and (n < 6):
                n = n + 1
                Source.drop(columns = ['FOR WHAT IS TRANSFERRED', 'RECEIVER TRI ID'],
                                inplace = True)
                Source.rename(columns = {'RECEIVER FRS ID': 'SENDER FRS ID'}, inplace =  True)
                Source = pd.merge(Source, Quantity, how = 'inner', on = 'SENDER FRS ID')
                ...
                try:
                    ...
                    if not Paths_aux.empty:
                        Paths_aux.rename(columns = {'RECEIVER FRS ID': 'FINAL HANDLER FRS ID',
                                                'RECEIVER TRI ID': 'FINAL HANDLER TRI ID'},
                                    inplace = True)
            ...
            Paths = Paths.loc[pd.notnull(Paths['FINAL HANDLER TRI ID'])]
            Paths['OFF-SITE WASTE MANAGEMENT'] = Management
            Paths['RECEIVER FRS ID'] = Receiver_FRS_ID
            Paths['SRS CHEMICAL ITN'] = Chemical_SRS_ID
    ...
    def Flows_Search(self):
        # Calling database
        TRI = pd.read_csv(self._dir_path + '/' + self.year + '/TRI_SRS_FRS_' + self.year + '_EoU.csv',
        ...
        non_hazardous = RCRA_facility.loc[pd.isnull(RCRA_facility['RCRAInfo CHEMICAL ID NUMBER']) \
                                         & pd.notnull(RCRA_facility['RECEIVER TRI ID'])]
        ...
        for F in Files:
            r = pd.read_csv(Path_RCRA + F, header = 0, sep = ',', low_memory = False,
                    usecols = ['REPORTING YEAR', 'SRS INTERNAL TRACKING NUMBER',
                        ...
                        'RECEIVER TRI ID', 'RECEIVER FRS ID'])
            r = r.loc[(r['QUANTITY RECEIVED'] != 0) & (pd.notnull(r['QUANTITY RECEIVED']))]
            r.rename(columns = {'SRS INTERNAL TRACKING NUMBER': 'SRS CHEMICAL ITN',
                                'FOR WHAT IS TRANSFERRED': 'RCRA Waste Management',
                                'REPORTING YEAR': 'RECEIVING YEAR',
                                'RECEIVER FRS ID': 'FINAL HANDLER FRS ID',
                                'RECEIVER TRI ID': 'FINAL HANDLER TRI ID'},
                    inplace = True)
            RCRAINFO = pd.concat([RCRAINFO, r], ignore_index = True, axis = 0)
            del r
        RCRA_facility = pd.merge(RCRA_facility, RCRAINFO, how = 'left',
                                left_on = ['SRS CHEMICAL ITN', 'RECEIVER TRI ID', 'RCRA Waste Management'],
                                right_on = ['SRS CHEMICAL ITN', 'FINAL HANDLER TRI ID', 'RCRA Waste Management']) \
                          .drop_duplicates(keep = 'first')
        del RCRAINFO, Files
        ...
        RCRA_facility.drop(['TRI Waste Management', 'RCRA Waste Management'],
                        axis = 1, inplace = True)
        No_successful = RCRA_facility.loc[pd.isnull(RCRA_facility['FINAL HANDLER TRI ID'])]
        RCRA_facility = RCRA_facility.loc[pd.notnull(RCRA_facility['FINAL HANDLER TRI ID'])]
        ...
        grouping = columns_grouping + ['TYPE OF WASTE MANAGEMENT','GENERAL WASTE MANAGEMENT' , \
                  'FINAL HANDLER TRI ID', 'FINAL HANDLER FRS ID']
        ...
        No_successful.drop(columns = ['GENERAL WASTE MANAGEMENT', 'TYPE OF WASTE MANAGEMENT',
                                    ...
                                    'FINAL HANDLER FRS ID', 'FINAL HANDLER TRI ID',
        ...
        TRI_facility['FINAL HANDLER FRS ID'] = TRI_facility['RECEIVER FRS ID']
        TRI_facility['FINAL HANDLER TRI ID'] = TRI_facility['RECEIVER TRI ID']
        ...
        df_TRI = df_TRI.loc[df_TRI.groupby(grouping, as_index = False)\
                                              .Year_difference.idxmin()] # Selecting between years
        del grouping
        df_TRI['TEMPORAL CORRELATION OF FINAL HANDLER'] = df_TRI.Year_difference.apply(lambda x: self._temporal_correlation(x))
        df_TRI.drop(['Year_difference'], axis = 1, inplace = True)
        df_TRI.rename(columns = {'REPORTING YEAR': 'FINAL HANDLER REPORTING YEAR'}, inplace = True)
        ...
        df_TRI.rename(columns = {'NAICS Title': 'FINAL HANDLER PRIMARY NAICS TITLE',
                                'PRIMARY NAICS CODE': 'FINAL HANDLER PRIMARY NAICS CODE',
                                'TOTAL WASTE': 'TOTAL WASTE GENERATED',
                                'TOTAL WASTE RELIABILITY': 'TOTAL WASTE GENERATED RELIABILITY',
                                'MAXIMUM AMOUNT ON-SITE': 'FINAL HANDLER MAXIMUM AMOUNT ON-SITE',
                                'CAS NUMBER': 'TRI CHEMICAL ID NUMBER',
                                'TRIFID': 'FINAL HANDLER TRI ID',
                                'FACILITY NAME': 'FINAL HANDLER NAME',
                                'FACILITY STREET': 'FINAL HANDLER STREET',
                                'FACILITY CITY': 'FINAL HANDLER CITY',
                                'FACILITY COUNTY': 'FINAL HANDLER COUNTY',
                                'FACILITY STATE': 'FINAL HANDLER STATE',
                                'FACILITY ZIP CODE': 'FINAL HANDLER ZIP'},
                    inplace = True)
        ...
        No_broker['MAXIMUM POSSIBLE FLOW RELIABILITY'] = No_broker.apply(lambda row: \
                                            ...
                                            axis = 1)
        No_broker['TEMPORAL CORRELATION OF MAXIMUM POSSIBLE FLOW'] = No_broker['TEMPORAL CORRELATION OF FINAL HANDLER']
        ...
        cols = cols[0:28] +  ['GENERAL WASTE MANAGEMENT', 'TYPE OF WASTE MANAGEMENT'] + \
            cols[28:] + ['FINAL HANDLER FRS ID', 'FINAL HANDLER TRI ID', \
            'FINAL HANDLER NAME', 'FINAL HANDLER STREET', 'FINAL HANDLER CITY', 'FINAL HANDLER COUNTY', \
            'FINAL HANDLER STATE', 'FINAL HANDLER ZIP', 'FINAL HANDLER PRIMARY NAICS CODE', \
            'FINAL HANDLER PRIMARY NAICS TITLE', 'FINAL HANDLER REPORTING YEAR', 'TEMPORAL CORRELATION OF FINAL HANDLER', \
            'FINAL HANDLER MAXIMUM AMOUNT ON-SITE', 'TOTAL WASTE GENERATED', 'TOTAL WASTE GENERATED RELIABILITY', \
        ...
        No_broker.to_csv(self._dir_path + '/' + self.year + '/TRI_SRS_FRS_off_site_tracking_and_compartments_' + self.year + '_EoU.csv',
                                   sep = ',', index = False)
        del No_broker
        #------------------------------- Brokers -------------------------------#
                ...
                if not df.empty:
                    ...
                    df['TEMPORAL CORRELATION OF MAXIMUM POSSIBLE FLOW'] = \
                                df.apply(lambda x: max([x['TEMPORAL CORRELATION OF MAXIMUM POSSIBLE FLOW'],
                                                        x['TEMPORAL CORRELATION OF FINAL HANDLER']]),
                                                        axis =  1)
                    df = self._Normalizing_NAICS(df)
                    df = df[cols]
                    df.to_csv(self._dir_path + '/' + self.year + '/TRI_SRS_FRS_off_site_tracking_and_compartments_' + self.year + '_EoU.csv',
    ...
    def CompTox_TSCA_Groups(self):
        ...
        # Calling database
        TRI = pd.read_csv(self._dir_path + '/' + self.year + '/TRI_SRS_FRS_off_site_tracking_and_compartments_' + self.year + '_EoU.csv',
                                 header = 0, sep = ',', low_memory = False,
        ...
        TRI.to_csv(self._dir_path + '/' + self.year + '/TRI_SRS_FRS_off_site_tracking_and_compartments_CompTox_' + self.year + '_EoU.csv',
                                 sep = ',', index = False)


    def _searching_equivalent_naics(self, df, naics):
        year = df['FINAL HANDLER REPORTING YEAR'].iloc[0]
        if year >= 2017:
            return df
        else:
            if (year < 2017) and (year >= 2012):
                df = pd.merge(df, naics[['2012 NAICS Code', '2017 NAICS Code', '2017 NAICS Title']],
                            how = 'left', left_on = 'FINAL HANDLER PRIMARY NAICS CODE',
            ...
            df['FINAL HANDLER PRIMARY NAICS CODE'].loc[idx] = df['2017 NAICS Code'].loc[idx]
            df['FINAL HANDLER PRIMARY NAICS TITLE'].loc[idx] = df['2017 NAICS Title'].loc[idx]
    ...
    def _Normalizing_NAICS(self, TRI):
        ...
        for i, file in enumerate(naics_files):
            ...
            if i != 0:
                ...
                df = df_aux
        TRI.sort_values(by = ['FINAL HANDLER REPORTING YEAR'], inplace = True)
        TRI.reset_index(inplace = True)
        TRI = TRI.groupby('FINAL HANDLER REPORTING YEAR', as_index = False).apply(lambda x: self._searching_equivalent_naics(x, df))
        TRI['FINAL HANDLER PRIMARY NAICS CODE'] = TRI['FINAL HANDLER PRIMARY NAICS CODE'].astype(pd.Int32Dtype())
        return TRI
...
if __name__ == '__main__':
    ...
    if args.Option == 'A':
        TRI = TRI_EoU(TRIyear, Files = TRIfiles)
